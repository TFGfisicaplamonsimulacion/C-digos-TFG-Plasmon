...
def Multicapa_Matrix(ldo: float, mats: list, d: float, theta_i: float, Polarization = 'TM', Campos = False):

    N, M, K, index_3D, sin_theta_r, cos_theta_r, u, g = Common_Matrix(ldo = ldo, mats = mats, d = d, theta_i = theta_i, Polarization = Polarization)
...
def Common_Matrix(ldo: float, mats: list, d: float, theta_i: float, Polarization: str):
    ...
    index_eval = np.array([ri.refindex(ldo, mat) for mat in mats])
    index_0 = np.array([ri.refindex(ldo, mats[0]) for mat in mats])
