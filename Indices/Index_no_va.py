...
sell_res = dict(        BK7 =  [[1.03961212, 0.231792344, 1.01046945],
                                [0.00600069867, 0.0200179144, 103.560653],
                                [0.3, 2.5]],
                    
                FS_Malitson =  [[0.6961663, 0.4079426, 0.8974794],
                ...
                GGG =          [[1.7727, 0.9767, 4.9668],
                                [0.1567**2, 0.01375**2, 22.715**2],
                                [0.2, 1.8]],
                
                HK9 =          [[0.614555251, 0.656775017, 1.02699346],
                                [0.0145987884, 0.00287769588, 107.653051],
                                [0.3, 2.5]],
                
                H_ZF13 =       [[1.73521591, 0.316277446, 2.16384634],
                                [0.0131087904, 0.0621421663, 178.845558],
                                [0.37, 2.4]],
                ...
                SF11 =         [[1.73759695, 0.313747346, 1.89878101],
                                ...
                                [0.37, 2.5]],
                Sapphire_no =  [[1.4313493, 0.65054713, 5.3414021],
                                [0.0726631**2, 0.1193242**2, 18.028251**2],
                                [0.2, 5.]],
                
                Sapphire_ne =  [[1.5039759, 0.55069141, 6.5927379],
                                [0.0740288**2, 0.1216529**2, 20.072248**2],
                                [0.2, 5.]])
...
exp_dis = dict( 
                ...
                Au_bulk    = './Raw_data/JohnsonAu.txt',
                Au_44nm    = './Raw_data/44nmAu.txt',
                
                Cargille_Acrylic = './Raw_data/Acrylic_MatchingLiquid_Cargille.txt',
                Cargille_FS = './Raw_data/FS_MatchingLiquid_Cargille.txt',
                
                Cr_bulk    = './Raw_data/JohnsonCr.txt',
                Cr_12nm    = './Raw_data/12nmCr_Sytchkova.txt',
                
                Cu_bulk    = './Raw_data/Cu_McPeak.txt',

                Li_bulk    = './Raw_data/Li_Rasigni.txt',
                
                MgF2_44nm  = './Raw_data/44nmMgF2.txt',
                
                NaCl_bulk  = './Raw_data/NaCl_Querry.txt',
                
                Pt_80nm =  './Raw_data/Pt_Tselin_80nm.txt',
                
                Sapphire_20nm = './Raw_data/Al2O3_Zhukovsky_20nm.txt',
                
                Si         = './Raw_data/Si_Schinke2015.txt',
                
                SiO2_56nm  = './Raw_data/SiO2_Rodriguez_56nm.txt',
                
                TiO2_20nm = './Raw_data/TiO2_Zhukovsky_20nm.txt',
                
                Water_20   = './Raw_data/Auga_Kedenburg.txt',
                
                ZnO_bulk    = './Raw_data/ZnO_Aguilar.txt',
                ZnO_40nm    = './Raw_data/40nm_ZnO_Stelling.txt',)


#%%=============================================================================
# Funcion main
# =============================================================================
def refindex(ldo: float, material: str,tempera:float, mod: Optional[str] = None) -> float:
    '''
    Parameters
    ----------
    ldo : float
    ...
    ldo = np.array(ldo)
    x = ldo*1e-3
    n = np.zeros(np.shape(x),dtype=np.complex_)
    ...
    if np.count_nonzero(n):
        ...
        # Casos multiples (varias referencias)
            ...
            case 'au' | 'ag' | 'al' | 'cr' | 'cu' | 'zno' | 'li' | 'nacl' | 'ggg' | 'h_zf13':
            ...
            case 'mgf2':
                ...
                n = checkMODEL(x, n, name)
            case 'sapphire':
                name = material + '_' + str(mod or 'no')
                n = checkMODEL(x, n, name)
            case 'tio2':
                name = material + '_' + str(mod or '20nm')
                n = checkMODEL(x, n, name)
            case 'sio2':
                name = material + '_' + str(mod or '56nm')
                n = checkMODEL(x, n, name)     
            case 'pt':
                name = material + '_' + str(mod or '80nm')
                n = checkMODEL(x, n, name)    
            case 'cargille':
                name = material + '_' + str(mod or 'fs')
                n = checkMODEL(x, n, name)
            
            case 'air':
                # Condiciones room*
                temperature = np.array(20).reshape(1) # Celsius
                pressure = np.array(101.3).reshape(1) # kPa
                humidity = np.array(60).reshape(1) # %
                method = 'ciddor'
                xCO2 = np.array(450).reshape(1) # ppm
                n += air_index(x*1e3, temperature, pressure, humidity, method, xCO2)

            case 'water':
                n += water_index(ldo, tempera)
            case 'void':
                n += 1
            case _:
...
def checkLDO(ldo, ldo_range):
    if np.max(ldo)>np.max(ldo_range) or np.min(ldo)<np.min(ldo_range):
...
def sellmeier(ldo, res):
    ...
    add = np.zeros(np.shape(ldo))

    for a,l2 in zip(res[0],res[1]):
       add += a*ldo**2 /(ldo**2 - l2)
       
    checkLDO(ldo,np.array(res[2]))
    
    return np.sqrt(1 + add)
...
def nk_txt(ldo:float, name: str) -> float:
    ...
    with open(dir_name + name, 'r') as file:
        ...
        for line in file:
            try:
                k = np.vstack([k,[float(h) for h in (line.rsplit())]])
            except:
                pass
    ...
    if not checkLDO(ldo,[min(n[1:,0]), max(n[1:,0])]):        
        n,k = np.interp(ldo,n[1:,0],n[1:,1]), np.interp(ldo,k[1:,0],k[1:,1])
...
def air_index(wavelength, temperature, pressure, humidity, method, xCO2):
    ...
    x1, x2, x3 = np.meshgrid(temperature, 1000 * pressure, humidity)
    ...
    psv2 = 611.657 * np.exp(Y)

    psv = np.where(x1 >= 0, psv1, psv2)
    pv = (humidity / 100) * psv
...
def water_index(wavelength, temperature):
    A= 1.3208 - 1.2325*1e-5 * temperature -1.8674*1e-6 * temperature**2 +5.0233*1e-9 * temperature ** 3
    B= 5208.2413 - 0.5179*temperature - 2.284*1e-2 * temperature **2 + 6.9608 *1e-5 * temperature **3
    C= -2.5551*1e8 - 18341.336*temperature - 917.2319 * temperature **2 + 2.7729 *temperature**3
    D=9.3495 + 1.7855*1e-3 * temperature + 3.6733*1e-5 *temperature **2 - 1.2932 * 1e-7 * temperature ** 3
    n= A + B/wavelength**2 + C/wavelength**4 +D/wavelength**6
    return(n)
